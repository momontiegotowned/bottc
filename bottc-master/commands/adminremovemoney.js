...
module.exports.run = async (bot, message, args) =>{
   // let logsCoin = bot.channels.cache.get('711554230661677056');
    ...
    Data.findOne({
        ...
        message.channel.send({embed:{color:'a20a28', description:`**${message.author.username}** removed ${args[1]}<:ttcchips:715103658601218088> to **${bot.users.cache.get(user.id).tag}**`}});
        dblogs.send({embed:{color:'a20a28', description:`**${message.author.username}** removed ${args[1]}<:ttcchips:715103658601218088> to **${bot.users.cache.get(user.id).tag}**`}});
        return;
